...
cli				; Clear interrupts
	...
	cmp dl, 0
	je no_change
	mov byte [bootdev], 0		; Save boot device number
	push es
...
main1:
	...
	mov bx, 25 ;delay on boot
	call delay
	
	call clear
	mov bx, 1 ;little delay to seem like something's going on... (nothings going on, it could take 0.001 seconds to load but who cares)
	call delay
...
parse: ;how we are going to do this:
	...
	mov di, peek_str
	call compare_string
	jc peek_string_found
	
	mov di, poke_str
	call compare_string
	jc poke_string_found
...
copy_do:
	...
	find2_copy:
		...
		jmp find2_copy
	done2_copy:
	;mov ax, input_buffer_copy ;how we copy files: load them, then get the filesize and point it at the file and make it under a new name
	;cx size_str
	;bx data
	;ax name
	;the variable [bootdev] controls what drive is being accessed. It has to be changed to either load or write the file. Say, if you wrote 'copy B:logo.pcx logo.pcx', B: declares change of bootdev 
	mov al, [tmp_dev_one]
	mov byte [bootdev], al
	call ready_the_drive
	mov ax, input_buffer_copy
...
check_for_diskette_two:

	cmp byte [si+2], 'A'
	je set_diskette_a_two

	mov byte [tmp_dev_two], 1
	mov byte [bootdev], 1
	check_two_done:
...
no_file_msg db 'File not found or none specified.', 0
...
skibidi db 'skibidi', 0 ;what was this for?
...
kernel_iden_two db 'KERNEL', 0
kernel_iden_thr db 'KERNEL', 0

entire_command times 60 db 0

previous_file_run times 13 db 0
input_buffer times 60 db 0
input_buffer_copy times 40 db 0
input_buffer_copy_copy times 40 db 0
super_iden db 'SUPER.BIN', 0
